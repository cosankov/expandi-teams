...
function makeContactRow(name, title, companyLine, imageUrl) {
  const column = {
    type: 'Column',
    width: 'stretch',
    items: [],
...
function formatReply(payload) {
  ...
  // Contact
  body.push(makeContactRow(name, title, companyLine, contact?.image_link));
...
function formatGenericAction(payload) {
  ...
  body.push(makeContactRow(name, contact?.job_title || '', companyLine, contact?.image_link));
